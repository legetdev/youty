...
enum FFmpegFrameExtractor {
    ...
    static func extract(
        ...
        //
        // Phase G (sample-table + parallel keyframe fetch + VTDecompression)
        // and Phase H (segment-batched seek loop with sidx-derived segment
        // map) were both built end-to-end and tested. Both regressed on
        // real DASH-fragmented YouTube videos:
        //  - Phase G delivers only 38 distinct frames (1 per DASH segment).
        //  - Phase H regresses 1-2 s on Rick and 4-12 s on long videos
        //    because for sparse targets each one lands in its own segment,
        //    making the batched loop equivalent to per-target plus overhead.
        //
        // The scaffolding (FFmpegSampleTable.swift, SidxParser.swift,
        // extractViaBatchedSeek) is retained for future codec-level work
        // (a real Phase H needs to decode B-frames out of order and would
        // benefit from HW decode pipelined with the GPU readback).
    ...
    // MARK: - Phase H: segment-batched seek loop

    private static func extractViaBatchedSeek(
        url: URL,
        userAgent: String,
        timestamps: [TimeInterval],
        maxLongEdge: Int32,
    ...
    private static func doExtractViaBatchedSeek(
        url: URL,
        userAgent: String,
        timestamps: [TimeInterval],
        maxLongEdge: Int32,
        ...
        // --- Setup: URLSession AVIO, AVFormatContext, codec (same as doExtract)
        ...
        guard avformat_open_input(&fmtCtxOpt, nil, nil, nil) == 0,
              let fmtCtx = fmtCtxOpt else { throw FFmpegError.openInputFailed(-1) }
        ...
        guard let stream = videoStream, videoStreamIndex >= 0,
              let codecPar = stream.pointee.codecpar else { throw FFmpegError.noVideoStream }

        guard let codec = avcodec_find_decoder(codecPar.pointee.codec_id) else {
            throw FFmpegError.codecNotFound
        }
        guard let codecCtx = avcodec_alloc_context3(codec) else { throw FFmpegError.codecNotFound }
        defer {
        ...
        // --- Phase H step 1: fetch segment map from sidx box (one HTTP
        // Range request for the first 16 KB of the file). Avoids the cost
        // of 100 av_seek_frame calls just to lazy-load FFmpeg's index.
        let sidxSegments: [SidxSegment] = try awaitSync {
            try await SidxParser.fetch(url: url, userAgent: userAgent)
        }
        let keyframes = sidxSegments.map { $0.pts }.sorted()
        guard !keyframes.isEmpty else { throw SampleTableError.noKeyframeIndex }

        // --- Phase H step 3: group sorted targets by segment (= containing keyframe)
        let sorted = timestamps.enumerated().sorted { $0.element < $1.element }
        struct SegmentJob { var keyframePts: Int64; var targets: [(inputIndex: Int, ts: TimeInterval, pts: Int64)] }
        var segments: [SegmentJob] = []
        for entry in sorted {
            let pts = toPTS(entry.element)
            // Greatest keyframe.pts ≤ pts (binary search)
            var lo = 0, hi = keyframes.count - 1, best = 0
            while lo <= hi {
                let mid = (lo + hi) / 2
                if keyframes[mid] <= pts { best = mid; lo = mid + 1 }
                else { hi = mid - 1 }
            }
            let kfPts = keyframes[best]
            if segments.last?.keyframePts == kfPts {
                segments[segments.count - 1].targets.append((entry.offset, entry.element, pts))
            } else {
                segments.append(SegmentJob(keyframePts: kfPts,
                                            targets: [(entry.offset, entry.element, pts)]))
            }
        }
        DebugLog.log("phase-H: \(timestamps.count) targets across \(segments.count) segments (keyframes=\(keyframes.count))")

        // --- Phase H step 4: decode per segment with one seek each
        var captures: [CapturedFrame?] = Array(repeating: nil, count: timestamps.count)
        var captured = 0
        for (segIdx, seg) in segments.enumerated() {
            _ = av_seek_frame(fmtCtx, videoStreamIndex, seg.keyframePts, AVSEEK_FLAG_BACKWARD)
            avcodec_flush_buffers(codecCtx)

            var unmatched = seg.targets
            decodeLoop: while !unmatched.isEmpty {
                av_packet_unref(packet)
                let r = av_read_frame(fmtCtx, packet)
                if r < 0 { break }
                if packet.pointee.stream_index != videoStreamIndex { continue }
                let s = avcodec_send_packet(codecCtx, packet)
                if s < 0 && s != EAGAIN_FFM { continue }

                while true {
                    let rcv = avcodec_receive_frame(codecCtx, avFrame)
                    if rcv == EAGAIN_FFM || rcv == AVERROR_EOF_FFM { break }
                    if rcv < 0 { throw FFmpegError.decodeFailed(rcv) }

                    let framePts = avFrame.pointee.best_effort_timestamp
                    // First unmatched target whose PTS ≤ framePts → capture
                    if !unmatched.isEmpty,
                       (framePts == AV_NOPTS_VALUE_FFM || framePts >= unmatched[0].pts) {
                        if let cap = captureFrame(from: avFrame, sws: swsCtx,
                                                  outW: outW, outH: outH) {
                            captures[unmatched[0].inputIndex] = cap
                            unmatched.removeFirst()
                            captured += 1
                            progress(Double(captured) / Double(timestamps.count))
                        }
                    }
                    av_frame_unref(avFrame)
                    if unmatched.isEmpty { break decodeLoop }
                }
            }
            // Drain any pending unmatched targets via decoder flush.
            if !unmatched.isEmpty {
                ...
                while !unmatched.isEmpty {
                    ...
                    if !unmatched.isEmpty,
                       (framePts == AV_NOPTS_VALUE_FFM || framePts >= unmatched[0].pts),
                       let cap = captureFrame(from: avFrame, sws: swsCtx,
                                              outW: outW, outH: outH) {
                        captures[unmatched[0].inputIndex] = cap
                        unmatched.removeFirst()
                        captured += 1
            ...
            if !unmatched.isEmpty {
                DebugLog.log("phase-H: segment \(segIdx) left \(unmatched.count) target(s) uncaught")
            }
        }

        // Reject incomplete extractions; force fallback to per-target loop.
        let missing = captures.enumerated().filter { $0.element == nil }.map { $0.offset }
        if !missing.isEmpty {
            throw FFmpegError.incompleteFrames(got: captures.count - missing.count, expected: captures.count)
        }
