...
Rust tokenizer — same tokens the Swift `GemmaTokenizer` produces — so query and
...
class EmbeddingGemmaTextEncoder:
    ...
    def _ensure_loaded(self) -> None:
        ...
        with self._lock:
            if self._model is not None:
                return
            from .query_tokenizers import load_gemma_tokenizer

            _log.info("Loading EmbeddingGemma tokenizer + Core ML model …")
            self._tok = load_gemma_tokenizer()
    ...
    def embed_query(self, text: str, *, dim: int = EMBEDDING_DIM) -> list[float]:
        ...
        ids = self._tok.encode(QUERY_PROMPT + text, add_special_tokens=True).ids
