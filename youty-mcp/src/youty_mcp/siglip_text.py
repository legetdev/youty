...
with `coremltools`, CPU) — no torch. Tokenization uses SentencePiece directly
with the original SigLIP normalization. Its text tower uses a fixed 64-token
...
class SigLIPTextEncoder:
    ...
    def _ensure_loaded(self) -> None:
        ...
        with self._lock:
            if self._model is not None:
                return
            from .query_tokenizers import SiglipQueryTokenizer

            _log.info("Loading SigLIP tokenizer + Core ML text model …")
            self._tok = SiglipQueryTokenizer()
